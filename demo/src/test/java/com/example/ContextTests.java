...
class ContextTests {
    ...
    void twoAndTwoIsFour() {
        try (Context context = Context.create()) {
            ...
            Assertions.assertEquals(4, js.execute(2, 2).asInt());

            Value py = context.eval("python", "lambda a, b : a + b");
            Assertions.assertTrue(py.canExecute());
            Assertions.assertEquals(4, py.execute(2, 2).asInt());
        }
    }
    ...
    void bewareLanguageSemantics() {
        try (Context context = Context.create()) {
            ...
            Assertions.assertTrue(js.canExecute());
            Assertions.assertEquals(4, js.execute(3).asInt());
            Assertions.assertEquals("31", js.execute("3").asString());
        }
        try (Context context = Context.create()) {
            Value python = context.eval("python", "lambda a : a + 1");
            Assertions.assertTrue(python.canExecute());
            Assertions.assertEquals(4, python.execute(3).asInt());
            Assertions.assertThrows(RuntimeException.class, () -> python.execute("3"));
        }
    }
    ...
    void incrementList() {
        try (Context context = Context.newBuilder("js").allowHostAccess(HostAccess.ALL).build()) {
            ...
            Collections.addAll(expected, 2, 3, 4, "41", "51", 7.5);
            Assertions.assertEquals(expected, objectList);
        }
    }
    ...
    void testMyCoolArray() {
        try (Context context = Context.newBuilder("js")
            ...
            Assertions.assertEquals(2, myCoolArray.get(0) instanceof Value ? ((Value) myCoolArray.get(0)).asInt() : myCoolArray.get(0));
            Assertions.assertEquals("11", myCoolArray.get(1) instanceof Value ? ((Value) myCoolArray.get(1)).asString() : myCoolArray.get(1));
            Assertions.assertEquals(2.1, myCoolArray.get(2) instanceof Value ? ((Value) myCoolArray.get(2)).asDouble() : myCoolArray.get(2));
            Assertions.assertEquals(3, myCoolArray.getSize());
        }
    }
    ...
    void testMyCoolArrayWithMyCoolObject() {
        try (Context context = Context.newBuilder("js").allowAllAccess(true).build()) {
            ...
            Assertions.assertTrue(js.canExecute());
            js.execute(myCoolArray);
            Assertions.assertEquals(2, ((MyCoolObject) myCoolArray.get(0)).value);
            Assertions.assertEquals("1_incrementedInJava", ((MyCoolObject) myCoolArray.get(1)).value);
            Assertions.assertEquals(2.1, ((MyCoolObject) myCoolArray.get(2)).value);
            Assertions.assertEquals(3, myCoolArray.getSize());
        }
    }
    ...
    void testLanguageSemanticsWork() {
        try (Context context = Context.newBuilder("js", "python").build()) {
            ...
            Assertions.assertEquals(1, ((MyCoolObject) myCoolArray.get(0)).value);
            Assertions.assertEquals("1", ((MyCoolObject) myCoolArray.get(1)).value);
            Assertions.assertEquals(1.1, ((MyCoolObject) myCoolArray.get(2)).value);
            Assertions.assertEquals(3, myCoolArray.getSize());

            Assertions.assertTrue(result.hasArrayElements());
            Assertions.assertEquals(2, result.getArrayElement(0).asInt());
            Assertions.assertEquals("11", result.getArrayElement(1).asString());
            Assertions.assertEquals(2.1, result.getArrayElement(2).asDouble());
            Assertions.assertEquals(3, myCoolArray.getSize());
        }
    }
    ...
    void testLanguageSemanticsFail() {
        try (Context context = Context.newBuilder("js", "python").build()) {
            ...
            Assertions.assertTrue(python.canExecute());
            Assertions.assertThrows(RuntimeException.class, () -> js.execute(myCoolArray, python));
        }
    }
}
